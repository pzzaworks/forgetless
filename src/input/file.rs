...
fn extract_pdf_text_fast(bytes: &[u8], path: &Path) -> String {
    ...
    if result.len() < 500 {
        ...
        for &b in bytes {
            if b.is_ascii_alphanumeric() || b == b' ' || b == b'.' || b == b',' || b == b'-' || b == b'\'' {
                word.push(b as char);
            } else if word.len() >= 2 {
...
pub fn read_file_preview(path: &Path) -> Option<String> {
    ...
    match extension.as_deref() {
        ...
        Some("png" | "jpg" | "jpeg" | "gif" | "webp" | "bmp") => {
            ...
            Some(format!("Image file: {}", path.file_name()?.to_string_lossy()))
        }
    ...
    match extension.as_deref() {
        ...
        Some("pdf") => {
            extract_pdf_with_pdftotext(path)
                .map(|text| (text, ContentType::Text))
        }
...
mod tests {
    ...
    fn test_read_valid_jpeg() {
        ...
        let jpeg_data: Vec<u8> = vec![
            ...
            0xBD, 0xC5, 0xDB, 0xB8, 0x71, 0xC4, 0x61, 0x71, 0x40, 0x17, 0x7F, 0xFF,
            0xD9,
        ];
